...
static lv_indev_t* p_indev = NULL;
...
void tpcal_create(lv_indev_t * indev )
{
   ...
   char buf[64];
   sprintf(buf, "Touch screen calibration\n"
           "Click to continue\n");
   ...
   // Replace callback with one that returns raw touchscreen readings
   
   indev->driver.read_cb = touchReadRaw;
   p_indev = indev;
}
...
static void btn_event_cb(lv_obj_t * scr, lv_event_t event)
{
   ...
   if(state == TP_CAL_STATE_INIT)
   {
   ...
   else if(state == TP_CAL_STATE_WAIT_TOP_LEFT) {
      char buf[64];
      touch_nb--;
      ...
      touchDoCalibration(point, CIRCLE_OFFSET);

      // restore touch callback
      p_indev->driver.read_cb = touchRead;
