...
void lv_tutorial_animations(void)
{
    ...
    label = lv_label_create(lv_disp_get_scr_act(NULL), NULL);
    lv_label_set_style(label, LV_LABEL_STYLE_MAIN, &lv_style_transp);
    lv_label_set_text(label, "Message here");
    lv_obj_set_pos(label, 10, 220);

    /* Animate the label */
    lv_anim_t sl;
    sl.var = label;
    sl.start = -lv_obj_get_width(label);
    sl.end = lv_disp_get_hor_res(NULL);
    sl.exec_cb = (lv_anim_exec_xcb_t)lv_obj_set_x;
    sl.path_cb = lv_anim_path_linear;
    sl.ready_cb = NULL;
    sl.act_time = -2000;                         /*Negative number to set a delay*/
    sl.time = 5000;                               /*Animate in 400 ms*/
    sl.playback = 0;                             /*Make the animation backward too when it's ready*/
    sl.playback_pause = 0;                       /*Wait before playback*/
    sl.repeat = 1;                               /*Repeat the animation*/
    sl.repeat_pause = 500;                       /*Wait before repeat*/
    lv_anim_create(&sl);
